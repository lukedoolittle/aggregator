...
namespace Aggregator.Test.Unit
{
    ...
    public class SchedulerTests
    {
        ...
        public void AddingANullTaskShouldThrowAnException()
        {
            ...
            Assert.Throws<ArgumentNullException>(() => actual.AddTask(Guid.NewGuid(), null, TimeSpan.Zero, false));
        }
            ...
            bool isSchedulerRunning)
        {
            var actual = new TaskMock();
            var taskId = Guid.NewGuid();
            var originalPollingInterval = TimeSpan.FromSeconds(5);
            var finalPollingInterval = TimeSpan.FromSeconds(10); ;
